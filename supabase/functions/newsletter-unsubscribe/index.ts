...
// Verifies the signed unsubscribe capability, sets newsletter_opt_out = true,
...
import { verifyNewsletterUnsubscribeToken } from '../_shared/newsletter-unsubscribe-capability.ts'
...
Deno.serve(async (req) => {
  ...
  let userId: string
  try {
    userId = await verifyNewsletterUnsubscribeToken(token)
  } catch {
    return new Response(
      htmlPage(
        'Invalid link',
