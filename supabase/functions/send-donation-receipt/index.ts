    ...
    try {
      ...
      await supabaseAdmin
        ...
        .update({
          ...
          receipt_sent_at: outcome === 'sent' ? new Date().toISOString() : null,
          receipt_last_error: outcome === 'failed' ? 'Receipt delivery failed' : null,
        })
        .eq('id', donationId)
        ...
        if (donationId) {
          await createClient(supabaseUrl, serviceKey)
            ...
            .update({
              ...
              receipt_last_error: 'Receipt delivery failed',
            })
            .eq('id', donationId)
        }
      } catch {
        /* best-effort state recovery */
      }
