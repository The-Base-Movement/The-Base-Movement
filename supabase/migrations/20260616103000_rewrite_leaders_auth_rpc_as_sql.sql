-- Rewrite the row-returning Leaders Auth RPCs as SQL functions.
-- This avoids PL/pgSQL RETURNS TABLE variable shadowing and makes the output
-- types unambiguous for PostgREST.
...
-- Force PostgREST to reload schema metadata so RPC signatures/body changes are
-- visible immediately.
