...
BEGIN
  ...
  -- Fingerprint mismatch: always block. User-agent strings are generic and not
  -- device-specific — different workstations with the same browser/OS send identical
  -- user-agents. A slot reset by IT is required to register a new workstation.
  ELSE
    v_decision     := 'blocked';
    v_log_action   := 'blocked';
