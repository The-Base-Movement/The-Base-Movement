-- Migration: Enforce Brave Browser for Tracked Admin Logins
-- 
-- 1. Update evaluate_admin_device to strictly block any non-Brave logins for tracked roles
-- 2. Update discord_leader_device_activity trigger to format alerts for 'non_brave_browser' blocks
...
BEGIN
  ...
  -- ── Enforce Brave Browser for tracked logins ──────────────────────────────
  IF COALESCE(p_browser, '') <> 'Brave' THEN
    -- Try to find device ID to attach to the activity log if a slot exists
    SELECT id INTO v_device.id
  ...
  -- ── Normal slot evaluation if browser is Brave ──────────────────────────────
  ELSE
...
-- Update trigger function to handle non_brave_browser reason in Discord alerts
