-- Align device-binding RPC signatures with the deployed edge functions and
-- enforce step-up verification when an enrolled device appears on a new ISP.
...
BEGIN
  ...
  ELSIF v_device.fingerprint_hash = p_fingerprint_hash THEN
    IF v_device.webauthn_enrolled AND COALESCE(v_device.isp, '') <> COALESCE(p_isp, '') THEN
  ...
  INSERT INTO public.admin_device_activity (
    ...
    jsonb_build_object('fingerprint_hash', p_fingerprint_hash, 'isp', p_isp)
  );
