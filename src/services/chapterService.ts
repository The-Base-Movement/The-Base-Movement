...
class ChapterService {
  ...
  async approveJoinRequest(
    ...
    void Promise.resolve(
      supabase.from('chapters').select('leader_id').eq('name', chapterName).maybeSingle()
    )
  ...
  async getRegionalLeaderboard(): Promise<ChapterLeaderboard[]> {
    try {
      // chapter_performance_telemetry is security_invoker; the SECURITY
      // DEFINER RPC keeps the public Impact leaderboard readable for anon
      const { data, error } = await supabase
        .rpc('get_chapter_performance')
        .select('*')
      ...
      const rows = (data ?? []) as Array<{
        ...
        total_chapter_achievements: number
        regional_chapter_rank: number
      }>
