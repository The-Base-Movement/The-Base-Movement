...
class LogisticsService {
  ...
  async getStoreProducts(): Promise<Product[]> {
    ...
    return (data || []).map((i) => ({
      ...
      price: `₵${i.price_ghs}`,
      description: i.description || 'Official movement gear. Designed for compatriots.',
      status: i.status,
      category: i.category,
      rating: i.rating || 4.8,
      reviews: i.reviews || 0,
      image: i.image_url,
      longDescription:
        i.long_description || i.description || 'Official movement gear. Designed for compatriots.',
  ...
  async getProductBySlug(slug: string): Promise<Product | null> {
    ...
    return {
      ...
      description: typedData.description || 'Official movement gear. Designed for compatriots.',
  ...
  /**
   * Order lookup for the order summary page. Tries the normal RLS read first
   * (admins and the order's owner); for guest-checkout orders (no customer_id)
   * falls back to the get_guest_order RPC, which returns a single order by its
   * UUID without exposing other guest orders.
   */
  async getOrderSummary(orderId: string): Promise<Order | null> {
    ...
    try {
      ...
      console.error('[DATABASE] Failed to fetch guest order:', error)
      return null
    }
  }
