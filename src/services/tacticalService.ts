...
class TacticalService {
  ...
  async getLeaderboard(region?: string): Promise<LeaderboardEntry[]> {
    try {
      // movement_leaderboard is security_invoker; the SECURITY DEFINER RPC
      // aggregates across members regardless of the caller's row access
      let query = supabase
        .rpc('get_movement_leaderboard')
  ...
  async getMovementPulse(): Promise<MovementPulse> {
    try {
      const [leaderboardRes, chaptersRes, velocityRes] = await Promise.all([
        supabase.rpc('get_movement_leaderboard').select('total_points, region'),
        supabase.rpc('get_chapter_performance').select('*'),
        supabase.from('logistics_velocity').select('fulfillment_rate'),
      ])

      const leaderboard = (leaderboardRes.data ?? []) as Array<{
        total_points: number
        region: string
      }>
      const chapters = (chaptersRes.data ?? []) as Array<{
        ...
        aggregate_chapter_points: number
      }>
