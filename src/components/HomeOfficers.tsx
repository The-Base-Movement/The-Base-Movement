...
export function HomeOfficers() {
  ...
  const [topTier, setTopTier] = useState<{ id: string, name: string, title: string, description: string, order_index: number } | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function loadOfficials() {
      const { data: tiersData } = await supabase.from('party_tiers').select('*').order('order_index', { ascending: true }).limit(1);
  ...
  return (
    <section aria-labelledby="officers-heading" className="py-16 md:py-24 bg-white border-b border-border/30">
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8">
        <ScrollReveal duration={850} distance="25px" direction="up">
          <div className="mb-10 md:mb-12">
            ...
            <h2 id="officers-heading" className="text-2xl md:text-3xl font-meta font-bold text-on-surface tracking-tight mb-1">
              {topTier?.title || 'National Executives'}
            </h2>
            <p className="text-sm text-muted-foreground mt-2 max-w-2xl font-body-md">
              {topTier?.description || 'Meet the dedicated executives driving our strategic vision and national mobilization.'}
            </p>
          </div>
        </ScrollReveal>

        <ScrollReveal duration={1000} delay={150} distance="35px" direction="up" style={{ marginBottom: '40px' }}>
          <OfficerCardSlider>
