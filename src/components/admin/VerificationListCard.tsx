...
export default function VerificationListCard({ member, isActive, onClick }: Props) {
  ...
  return (
    ...
    >
      {/* Row 1: avatar · name + submitted · status */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 42, height: 42, borderRadius: 4, overflow: 'hidden', background: isActive ? 'rgba(255,255,255,.1)' : '#f1f5ee', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: isActive ? '1px solid rgba(255,255,255,.15)' : '1px solid hsl(var(--border))' }}>
          {member.photoUrl
            ...
            : <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 13, color: isActive ? 'rgba(255,255,255,.6)' : 'hsl(var(--on-surface-muted))' }}>{initials}</span>
          }
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ margin: 0, fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 13.5, color: isActive ? '#fff' : 'hsl(var(--on-surface))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {member.name}
          </p>
          <span style={{ fontSize: 10.5, color: isActive ? 'rgba(255,255,255,.45)' : 'hsl(var(--on-surface-muted))', fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
            {member.id} · {member.submitted}
          </span>
        </div>
        {isActive
          ? <span style={{ padding: '2px 9px', background: 'rgba(255,255,255,.12)', border: '1px solid rgba(255,255,255,.2)', borderRadius: 99, fontSize: 9.5, fontFamily: "'Public Sans', sans-serif", fontWeight: 800, color: '#fff', textTransform: 'uppercase', letterSpacing: '.05em', flexShrink: 0 }}>{member.status}</span>
          : <span className={statusPill(member.status)} style={{ flexShrink: 0 }}>{member.status}</span>
        }
      </div>

      {/* Row 2: region · constituency */}
      {(member.region || member.constituency) && (
        <div style={{ ...metaStyle, color: isActive ? 'rgba(255,255,255,.45)' : 'hsl(var(--on-surface-muted))' }}>
          <span className="material-symbols-outlined" style={{ fontSize: 13 }}>place</span>
          {member.region}{member.constituency && member.constituency !== '—' ? ` · ${member.constituency}` : ''}
        </div>
      )}
    </div>
  )
