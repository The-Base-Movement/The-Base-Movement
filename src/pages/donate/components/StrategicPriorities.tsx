...
export function StrategicPriorities({
  ...
  // 4 cards per page on single-column mobile, 6 on tablet/desktop grids
  const isMobile = useMediaQuery('(max-width: 540px)')
  const pageSize = isMobile ? 4 : 6

  const totalPages = Math.max(1, Math.ceil(campaigns.length / pageSize))
  // Clamp at render time so a resize that shrinks totalPages can't strand us
  // on a page that no longer exists
  const safePage = Math.min(page, totalPages - 1)
  const paginated = campaigns.slice(safePage * pageSize, safePage * pageSize + pageSize)
          ...
          {Array.from({ length: totalPages }).map((_, i) => (
            <button
              ...
              style={{
                width: 36,
                height: 36,
                border: `1px solid ${i === safePage ? 'hsl(var(--primary))' : 'hsl(var(--border))'}`,
                borderRadius: 'var(--radius-sm)',
                background: i === safePage ? 'hsl(var(--primary))' : 'hsl(var(--background))',
                color: i === safePage ? '#fff' : 'hsl(var(--on-surface-muted))',
                fontFamily: "'Public Sans', sans-serif",
                fontWeight: 'var(--font-weight-medium, 500)',
                fontSize: 13,
                cursor: 'pointer',
