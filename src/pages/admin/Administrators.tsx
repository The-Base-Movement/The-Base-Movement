...
export default function Administrators() {
  ...
  const [needsLoginEmail, setNeedsLoginEmail] = useState(false)
  ...
  useEffect(() => {
    ...
    adminService
      .checkAdminLoginEmail(memberId)
      .then(({ needsEmail, currentEmail }) => {
        if (cancelled) return
        setNeedsLoginEmail(needsEmail)
        if (needsEmail && currentEmail) setProvisionEmail(currentEmail)
      })
      .catch(() => {
        /* non-blocking — provisioning proceeds without the email step */
      })
    return () => {
      cancelled = true
      clearTimeout(timer)
  ...
  const handleProvision = async () => {
    ...
    try {
      // Assign the login email first: if the email is taken or invalid we
      // abort before any admin role is granted
      if (needsLoginEmail) {
        await adminService.assignAdminLoginEmail(memberId, provisionEmail.trim())
      }
    ...
    <div className="main" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {showProvision && (
        <ProvisionModal
          ...
          needsLoginEmail={needsLoginEmail}
          provisionEmail={provisionEmail}
          setProvisionEmail={setProvisionEmail}
        />
      )}
