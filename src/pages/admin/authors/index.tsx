  ...
  return (
    <div className="main">
        ...
        <div>
          {formView && (
            <div className="crumbs" style={{ marginBottom: 6 }}>
              <button onClick={() => setFormView(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, color: 'hsl(var(--primary))', fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 12 }}>
                Authors directory
              </button>
              {' · '}
              <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 12, color: 'hsl(var(--on-surface-muted))' }}>
                {formView.mode === 'new' ? 'Recruit author' : 'Edit author'}
              </span>
            </div>
          )}
          <h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="material-symbols-outlined" style={{ fontSize: 24 }}>{formView ? (formView.mode === 'new' ? 'person_add' : 'manage_accounts') : 'edit_note'}</span>
            {formView ? (formView.mode === 'new' ? 'Recruit new personnel' : 'Refine credentials') : 'Authors directory'}
          </h2>
          <div className="bl"><div /><div /><div /></div>
          {!formView && <p style={{ fontFamily: "'Public Sans', sans-serif", fontSize: 13, color: 'hsl(var(--on-surface-muted))', marginTop: 8 }}>Editorial intelligence — verified content creators.</p>}
        </div>
        {!formView && (
      ...
      <div className="kpis">
        ...
        <TacticalKPI label="Mobility" value={availableRoles.length} description="Distinct tactical roles" trend={{ direction: 'up', value: 'Diverse' }} />
        <TacticalKPI label="Published posts" value={publishedPostCount ?? '—'} description="Live blog posts" trend={{ direction: 'up', value: 'Content' }} />
        <TacticalKPI label="Appointed" value={authors.filter(a => a.memberId).length} description="Linked to members" trend={{ direction: 'neutral', value: 'Verified' }} />
      </div>

      <div className="sidebar-main">
        {/* Sidebar */}
        <aside className="desktop-only panel" style={{ padding: 0, overflow: 'hidden', alignSelf: 'start', position: 'sticky', top: 80 }}>
          <div style={{ padding: '14px 18px', borderBottom: '1px solid hsl(var(--border))', background: 'hsl(var(--container-low))', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 13, color: 'hsl(var(--on-surface))' }}>Filter authors</span>
          </div>
          <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div>
              <label htmlFor="input-650b52" style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--on-surface-muted))', display: 'block', marginBottom: 6 }}>Filter authors</label>
            ...
            <div>
              <label htmlFor="select-b631d5" style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--on-surface-muted))', display: 'block', marginBottom: 6 }}>Role classification</label>
            ...
            <div style={{ paddingTop: 14, borderTop: '1px solid hsl(var(--border))' }}>
              <div style={{ background: 'hsl(var(--container-low))', border: '1px solid hsl(var(--border))', borderRadius: 4, padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, color: 'hsl(var(--primary))' }}>
                  ...
                  <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11 }}>Protocol V01</span>
                </div>
        ...
        <div className="panel overflow-hidden">
                    ...
                    <tr key={author.id}>
                      <td>
                        <div className="who">
                          <div style={{ width: 36, height: 36, borderRadius: '50%', border: '1px solid hsl(var(--border))', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'hsl(var(--container-low))' }}>
                            <img src={author.imageUrl || '/author-placeholder.svg'} crossOrigin="anonymous" loading="lazy" alt={author.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          </div>
                          <div>
                            <b style={{ cursor: 'pointer' }} onClick={() => setViewingAuthor(author)}>{author.name}</b>
                            <span className="text-[10px] font-bold opacity-50">{author.id.substring(0, 8)}</span>
                          </div>
                        </div>
                      </td>
                      <td>
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                          ...
                          {author.memberId && (
                            <span className="pill" style={{ fontSize: 10, background: 'hsla(var(--accent), 0.12)', color: 'hsl(var(--accent))', borderColor: 'hsla(var(--accent), 0.3)' }}>
                              <span className="material-symbols-outlined" style={{ fontSize: 10 }}>how_to_reg</span> Appointed
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td style={{ textAlign: 'right' }}>
                        <div className="row-actions justify-end">
                          <button onClick={() => setViewingAuthor(author)} className="ico" title="View"><span className="material-symbols-outlined" style={{ fontSize: 16 }}>visibility</span></button>
                          <button onClick={() => setFormView({ mode: 'edit', authorId: author.id })} className="ico" title="Edit"><span className="material-symbols-outlined" style={{ fontSize: 16 }}>edit</span></button>
                          <button onClick={() => setDeleteConfirm({ id: author.id, name: author.name })} className="ico no" title="Delete"><span className="material-symbols-outlined" style={{ fontSize: 16 }}>delete</span></button>
                        </div>
                      </td>
                    </tr>
                  ))
          ...
          {/* Mobile card list */}
          <div className="mobile-only" style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {isLoading ? (
              Array(4).fill(0).map((_, i) => (
                <div key={i} className="panel" style={{ padding: '14px 16px 14px 20px', position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: 'hsl(var(--border))' }} />
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'hsl(var(--border))', flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ width: '60%', height: 12, background: 'hsl(var(--border))', borderRadius: 3, marginBottom: 10 }} />
                      <div style={{ width: 70, height: 18, background: 'hsl(var(--border))', borderRadius: 99 }} />
                    </div>
                  </div>
                </div>
              ))
              ...
              <div style={{ padding: '48px 20px', textAlign: 'center' }}>
                <span className="material-symbols-outlined" style={{ fontSize: 36, color: 'hsl(var(--border))', display: 'block', marginBottom: 10 }}>edit_note</span>
                <p style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 13, color: 'hsl(var(--on-surface-muted))', marginBottom: 12 }}>No personnel matching search criteria</p>
                <button className="btn btn-outline btn-sm" onClick={() => { setSearchQuery(''); setRoleFilter('all') }}>Reset filters</button>
              </div>
            ) : (
              filteredAuthors.map(author => {
                ...
                return (
                  <div key={author.id} className="panel" style={{ overflow: 'hidden', position: 'relative' }}>
                    <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: isAppointed ? 'hsl(var(--accent))' : 'hsl(var(--primary))' }} />

                    {/* Main row */}
                    <div style={{ padding: '14px 16px 14px 20px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={() => setViewingAuthor(author)}>
                      <div style={{ width: 48, height: 48, borderRadius: '50%', border: '2px solid hsl(var(--border))', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'hsl(var(--container-low))' }}>
                        <img src={author.imageUrl || '/author-placeholder.svg'} crossOrigin="anonymous" loading="lazy" alt={author.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <b style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 14, color: 'hsl(var(--on-surface))', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{author.name}</b>
                        <div style={{ marginTop: 5, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          <span className="pill pill-ok" style={{ fontSize: 10 }}>{author.role || 'Contributor'}</span>
                          {isAppointed && (
                            <span className="pill" style={{ fontSize: 10, background: 'hsla(var(--accent), 0.12)', color: 'hsl(var(--accent))', borderColor: 'hsla(var(--accent), 0.3)' }}>
                              <span className="material-symbols-outlined" style={{ fontSize: 10 }}>how_to_reg</span> Appointed
                            </span>
                        ...
                        <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 10, color: 'hsl(var(--on-surface-muted))', display: 'flex', alignItems: 'center', gap: 3, marginTop: 5 }}>
                          <span className="material-symbols-outlined" style={{ fontSize: 11 }}>calendar_today</span>
                          {format(new Date(author.createdAt), 'MMM dd, yyyy')}
                        </span>
                      </div>
                      <span className="material-symbols-outlined" style={{ fontSize: 18, color: 'hsl(var(--on-surface-muted))', flexShrink: 0 }}>chevron_right</span>
                    </div>

                    {/* Action footer */}
                    <div style={{ padding: '10px 16px 10px 20px', borderTop: '1px solid hsl(var(--border))', background: 'hsl(var(--container-low))', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                      <button onClick={(e) => { e.stopPropagation(); setViewingAuthor(author) }} className="btn btn-outline btn-sm">
                        <span className="material-symbols-outlined" style={{ fontSize: 14 }}>visibility</span>
                        View
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); setFormView({ mode: 'edit', authorId: author.id }) }} className="btn btn-outline btn-sm">
                        <span className="material-symbols-outlined" style={{ fontSize: 14 }}>edit</span>
                        Edit
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ id: author.id, name: author.name }) }} className="btn btn-dest btn-sm" style={{ minWidth: 34, padding: '0 8px', justifyContent: 'center' }}>
            ...
            <>
              ...
              <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 70, background: '#fff', borderRadius: '14px 14px 0 0' }}>
                <div style={{ padding: '16px 18px', borderBottom: '1px solid hsl(var(--border))', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 14, color: 'hsl(var(--on-surface))' }}>Filter authors</span>
                ...
                <div style={{ padding: '18px 18px 32px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div>
                    <label htmlFor="mob-authors-search" style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 900, fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'hsl(var(--on-surface-muted))', display: 'block', marginBottom: 8 }}>Filter authors</label>
                    <div style={{ position: 'relative' }}>
                      ...
                      <input
                        id="mob-authors-search" name="mobAuthorsSearch"
                  ...
                  <div>
                    <label htmlFor="mob-authors-role" style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 900, fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'hsl(var(--on-surface-muted))', display: 'block', marginBottom: 8 }}>Role classification</label>
                    <select id="mob-authors-role" name="mobRoleFilter" value={roleFilter} onChange={e => setRoleFilter(e.target.value)} style={{ ...selectSt, height: 42 }}>
                      <option value="all">All roles</option>
                      {availableRoles.map(role => <option key={role} value={role}>{role}</option>)}
                    </select>
                  </div>
      ...
      <AuthorDetailModal
        ...
        onEdit={(id) => { setViewingAuthor(null); setFormView({ mode: 'edit', authorId: id }) }}
      />
    </div>
  )
}

function AuthorDetailModal({ author, isOpen, onClose, onEdit }: { author: Author | null, isOpen: boolean, onClose: () => void, onEdit: (id: string) => void }) {
      ...
      >
        ...
        <div style={{ padding: '36px 24px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--on-surface-muted))' }}>
                <span className="material-symbols-outlined" style={{ fontSize: 13, opacity: 0.5 }}>shield</span>
                Id: <span style={{ fontFamily: 'monospace', letterSpacing: 'normal' }}>{author.id.substring(0, 8)}</span>
              </span>
            </div>
          </div>
          ...
          <div style={{ borderTop: '1px solid hsl(var(--border))', paddingTop: 16 }}>
            <p style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--on-surface-muted))', margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <div style={{ background: 'hsl(var(--container-low))', borderTop: '1px solid hsl(var(--border))', padding: '16px 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div>
              <div style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--on-surface-muted))', marginBottom: 6 }}>Date enlisted</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 12, color: 'hsl(var(--on-surface))' }}>
                <span className="material-symbols-outlined" style={{ fontSize: 14, color: 'hsl(var(--primary))', opacity: 0.45 }}>calendar_today</span>
                {format(new Date(author.createdAt), 'MMM dd, yyyy')}
              </div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--on-surface-muted))', marginBottom: 6 }}>Security status</div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--primary))' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'hsl(var(--primary))', boxShadow: '0 0 8px rgba(0,107,63,0.4)' }} />
                Authorized duty
              </div>
            </div>
          </div>
          <div style={{ borderTop: '1px solid hsl(var(--border))', paddingTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 11, color: 'hsl(var(--on-surface-muted))' }}>
