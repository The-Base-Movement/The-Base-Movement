  ...
  const handleSubmitRegistration = async (data: RegistrationSubmission) => {
    ...
    try {
      const newUser = {
        ...
        job_industry_id: data.job?.industryId ?? null,
        job_sub_category_id: data.job?.subCategoryId ?? null,
        job_role_id: data.job?.isOther ? null : (data.job?.roleId ?? null),
        job_custom_title: data.job?.isOther ? data.job.customTitle.trim() || null : null,
