      ...
      <div className="grid-stats mb-12" style={{ '--grid-min-width': '220px' } as React.CSSProperties}>
        <Card className="rounded-sm border-border/60 shadow-sm bg-white">
          <CardContent className="p-6 flex flex-col gap-1">
            <p className="text-tiny font-bold text-muted-foreground/80 tracking-tight">Total Contributions</p>
            <h3 className="text-xl md:text-2xl font-bold text-on-surface">{stats.totalContributions.toLocaleString()}</h3>
            <span className="text-micro font-bold text-muted-foreground/60 mt-1">All-time recorded volume</span>
          </CardContent>
        </Card>
        <Card className="rounded-sm border-border/60 shadow-sm bg-white">
          <CardContent className="p-6 flex flex-col gap-1">
            <p className="text-tiny font-bold text-muted-foreground/80 tracking-tight">Pending Review</p>
            <h3 className="text-xl md:text-2xl font-bold text-accent">{stats.pendingCount.toLocaleString()}</h3>
            <span className="text-micro font-bold text-accent/70 mt-1">Awaiting administrative clearance</span>
          </CardContent>
        </Card>
        <Card className="rounded-sm border-border/60 shadow-sm bg-white">
          <CardContent className="p-6 flex flex-col gap-1">
            <p className="text-tiny font-bold text-muted-foreground/80 tracking-tight">Approved Amount</p>
            <h3 className="text-2xl font-bold text-primary">GH₵ {stats.approvedAmount.toLocaleString()}</h3>
            <span className="text-micro font-bold text-primary/70 mt-1">Total cleared funds</span>
          </CardContent>
        </Card>
        <Card className="rounded-sm border-border/60 shadow-sm bg-white">
          <CardContent className="p-6 flex flex-col gap-1">
            <p className="text-tiny font-bold text-muted-foreground/80 tracking-tight">Flagged Transactions</p>
            <h3 className="text-2xl font-bold text-destructive">{stats.flaggedCount.toLocaleString()}</h3>
            <span className="text-micro font-bold text-destructive/70 mt-1">Rejected or disputed records</span>
          </CardContent>
        </Card>
      </div>
                    ...
                    <CardContent className="p-0">
                      <div className="flex flex-col lg:flex-row items-stretch">
                        ...
                        <div className="p-6 lg:w-2/5 border-b lg:border-b-0 lg:border-r border-border/40 flex items-start gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-micro font-bold text-muted-foreground/60 bg-border/40 px-2 py-0.5 rounded-full">
                                {donation.reference.toUpperCase()}
                              </span>
                            </div>
                            <p className="text-micro text-muted-foreground/80 font-medium line-clamp-1">{donation.campaignTitle || 'General fund'}</p>
                            <div className="flex items-center gap-3 mt-3">
                              <span className="flex items-center gap-1.5 text-micro font-bold text-muted-foreground/80">
                        ...
                        <div className="p-6 lg:w-1/4 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-border/40 bg-muted/5">
                          <p className="text-micro font-bold text-muted-foreground/60 mb-1">Transaction value</p>
                          ...
                          <div className="mt-4">
                            {donation.receiptUrl ? (
                              ...
                              <span className="text-micro font-bold text-muted-foreground/60 flex items-center gap-1">
                        ...
                        <div className="p-6 lg:w-1/3 flex flex-col justify-center bg-white">
                          {donation.status === 'Pending' ? (
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                <p className="text-micro font-bold text-muted-foreground/60 mb-1">Audit status</p>
                                <span className={cn(
