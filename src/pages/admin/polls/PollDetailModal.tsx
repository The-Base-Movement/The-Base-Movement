      ...
      <div style={modalBox(580)} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="ph">
          <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 13.5, color: 'hsl(var(--on-surface))' }}>
            <span className="material-symbols-outlined" style={{ fontSize: 15, color: poll.status === 'Active' ? 'hsl(var(--primary))' : 'hsl(var(--on-surface-muted))' }}>
              bar_chart
            </span>
            Poll Details
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20 }}>
              {[
                ...
                {
                  icon: 'schedule',
                  label: poll.status === 'Active' ? `Closes in ${days} day${days !== 1 ? 's' : ''}` : 'End date',
                  value: poll.endDate,
                },
                ...
                <div key={option.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      ...
                      <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: isLead ? 800 : 700, fontSize: 12.5, color: 'hsl(var(--on-surface))' }}>
                        {option.label}
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 12, color: isLead ? 'hsl(var(--primary))' : 'hsl(var(--on-surface-muted))', minWidth: 36, textAlign: 'right' }}>
                        {pct}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div style={{ height: 6, background: 'hsl(var(--container-low))', borderRadius: 3, overflow: 'hidden', border: '1px solid hsl(var(--border))' }}>
                    <div style={{
                      height: '100%', width: `${pct}%`, transition: 'width 0.8s ease', borderRadius: 3,
          ...
          <div style={{ background: 'hsl(var(--container-low))', border: '1px solid hsl(var(--border))', borderRadius: 4, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 12, color: 'hsl(var(--on-surface))' }}>
              Total participation
            </span>
            <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 13, color: 'hsl(var(--primary))' }}>
        ...
        <div style={{ padding: '0 24px 24px', display: 'flex', gap: 10 }}>
          <button className="btn btn-dest btn-sm" style={{ justifyContent: 'center' }} onClick={() => { onClose(); onDelete(poll) }}>
            <span className="material-symbols-outlined" style={{ fontSize: 15 }}>delete</span>
            Delete Poll
          </button>
          <button className="btn btn-outline btn-sm" style={{ flex: 1, justifyContent: 'center' }} onClick={onClose}>
            Close
          </button>
        </div>
      </div>
