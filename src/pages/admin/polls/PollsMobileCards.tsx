...
export function PollsMobileCards({ polls, isLoading, searchQuery, onSearchChange, onView, onDelete }: PollsMobileCardsProps) {
  return (
    <div className="mobile-only" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {/* Mobile search bar */}
      <div className="panel">
        <div style={{ padding: 12, position: 'relative' }}>
          <span className="material-symbols-outlined" style={{ position: 'absolute', left: 22, top: '50%', transform: 'translateY(-50%)', fontSize: 15, color: 'hsl(var(--on-surface-muted))', pointerEvents: 'none' }}>
            search
          </span>
          <input
        ...
        polls.map((poll) => (
          <div key={poll.id} className="panel" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              <div>
                <div style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 13, color: 'hsl(var(--on-surface))', lineHeight: 1.4, marginBottom: 3 }}>
                  {poll.question}
                </div>
                <div style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 700, fontSize: 10.5, color: 'hsl(var(--on-surface-muted))' }}>
                  {poll.id}
                </div>
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 14, color: 'hsl(var(--on-surface))' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontFamily: "'Public Sans', sans-serif", fontWeight: 800, fontSize: 12, color: 'hsl(var(--on-surface))' }}>
                  {poll.endDate}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn" style={{ flex: 1, justifyContent: 'center', background: 'hsl(var(--accent))', color: '#fff' }} onClick={() => onView(poll)}>
                Manage Campaign
              </button>
              <button className="btn btn-dest" style={{ width: 44, padding: 0, justifyContent: 'center' }} onClick={() => onDelete(poll)}>
