    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-900">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-bold normal-case text-red-500 animate-pulse">Initializing war room protocols...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">Real-time intelligence, rapid response dispatch, and threat neutralization.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="rounded-xl border-stone-200 shadow-sm bg-white">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold normal-case text-stone-400 mb-1">Operational readiness</p>
              <h3 className="text-3xl font-bold text-red-600 tracking-tight">Level 2</h3>
            </div>
            ...
            <CardHeader className="p-6 border-b border-stone-100 bg-stone-50/50">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-[10px] font-bold normal-case text-stone-400 mt-1">Localized resistance and PR threats</CardDescription>
                </div>
              </div>
            </CardHeader>
              ...
              <div className="divide-y divide-stone-100">
                {incidents.length === 0 ? (
                  <div className="p-12 text-center">
                    ...
                    <p className="text-[10px] font-bold text-stone-400 normal-case">No active incidents. All sectors secure.</p>
                  </div>
                ) : (
                    ...
                    <div key={incident.id} className="p-6 hover:bg-stone-50 transition-colors">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-bold text-stone-500 normal-case">{incident.region}</span>
                        </div>
                        <span className="text-[9px] font-bold text-stone-400">{format(new Date(incident.created_at), 'MMM dd, HH:mm')}</span>
                      </div>
                      <h3 className="text-sm font-bold text-stone-900 normal-case mb-2">{incident.incident_type.replace('_', ' ').toLowerCase()}</h3>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-stone-100">
                        ...
                        <Button variant="outline" className="h-8 rounded-lg text-[9px] font-bold normal-case border-stone-200">
                          Update status
                        </Button>
                      </div>
                    </div>
              ...
              <div className="divide-y divide-stone-800">
                {narratives.length === 0 ? (
                  <div className="p-8 text-center">
                    <p className="text-[10px] font-bold text-stone-500 normal-case">No active media campaigns.</p>
                  </div>
                ) : (
                  narratives.map((nar) => (
                    <div key={nar.id} className="p-5 hover:bg-stone-800/50 transition-colors">
                      ...
                      <p className="text-sm text-stone-300 mb-3">"{nar.approved_messaging}"</p>
                      <div className="flex items-center justify-between">
                        <p className="text-[9px] font-bold text-stone-500 normal-case">{nar.hashtags}</p>
          ...
          <Card className="rounded-xl border-stone-200 shadow-sm bg-white overflow-hidden h-full">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-stone-100 max-h-[800px] overflow-y-auto">
                {directives.length === 0 ? (
                  <div className="p-8 text-center">
                    <p className="text-[10px] font-bold text-stone-400 normal-case">No active directives.</p>
                  </div>
                ) : (
                  directives.map((dir) => (
                    <div key={dir.id} className={cn(
                      ...
                      <h4 className="text-xs font-bold text-stone-900 normal-case mb-1">{dir.title}</h4>
                      <p className="text-[10px] text-stone-500 font-medium mb-3">{dir.action_type.replace('_', ' ').toLowerCase()}</p>
