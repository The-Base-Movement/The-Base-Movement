...
import { ShoppingBag, Heart, ArrowLeft, Trash2, ShoppingCart } from 'lucide-react'
  ...
  return (
    <div className="w-full px-6 md:px-12 py-12 bg-off-white min-h-screen">
      ...
      <main>
        {wishlist.length > 0 ? (
            ...
            {wishlist.map((item: Product) => (
              <div key={item.id} className="bg-white border border-stone-200 rounded-none overflow-hidden hover:shadow-xl transition-all duration-500 group">
                <div className="relative aspect-square bg-stone-50 flex items-center justify-center overflow-hidden">
                  <ShoppingBag className="w-16 h-16 text-stone-200 group-hover:scale-110 transition-transform duration-500" />
                  <button 
                ...
                <div className="p-6">
                  ...
                  <div className="pt-6 border-t border-stone-100 flex gap-3">
                    ...
                    <Button asChild variant="outline" className="flex-1 border-stone-200 hover:border-brand-green hover:text-brand-green text-[10px] font-bold uppercase tracking-widest rounded-none h-11">
                      <Link to={`/store/product/${item.slug}`}>Details</Link>
                    </Button>
                  </div>
                </div>
              </div>
          ...
          <div className="py-24 text-center bg-white border border-stone-200 rounded-none">
            ...
            <h2 className="font-h2 text-stone-400 mb-4 uppercase tracking-widest">Your wishlist is empty</h2>
            ...
            <Button asChild className="bg-brand-green hover:bg-emerald-700 text-white text-xs font-bold uppercase tracking-widest rounded-none px-12 h-12">
      ...
      <section className="mt-24">
        <h2 className="font-h2 text-h3 text-stone-900 mb-12 uppercase tracking-tight">You might also like</h2>
